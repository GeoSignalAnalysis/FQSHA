...
from src.FQSHA_Functions import kin2coeff, coeff2mag, conflate_pdfs, CHGaussBPT, CHGaussPoiss, TruncatedGR
import numpy as np
from scipy.stats import norm
...
def momentbudget(faults, Zeta, Khi, Siggma, ProjFol, logical_nan,  logical_nan_sdmag):
    ...
    for fault_name in faults:
        ...
        upperSeismoDepth = faults[fault_name]['upperSeismoDepth']
        lowerSeismoDepth = faults[fault_name]['lowerSeismoDepth']
        ...
        fault_rake = faults[fault_name]['fault_rake']
        fault_trace = faults[fault_name]['fault_trace']

        # # Calculate min and max latitude and longitude for each fault
        # for (lat, lon) in fault_trace:
        ...
        Seismogenic_thickness = float(lowerSeismoDepth - upperSeismoDepth)
        ...
        sine_Dip = math.sin(Dip_radians);        Length = Length * 1000;
